...
By default nothing survives a restart. For discovery data (observations, assets, relationships) and the hash-chained audit log (`GET /v1/audit`) to survive one, add `-db-driver sqlite -db-dsn <path>`:
...
Enrollment tokens, collector heartbeats, and job state remain in-memory only regardless of `-db-driver` — though the audit record that a token was issued, a collector enrolled, or a job created still persists. See [Persistent storage and the audit log](docs/storage.md).
