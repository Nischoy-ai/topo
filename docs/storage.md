...
Persisted (once `-db-driver sqlite` is configured): every ingested observation (`POST /v1/observations`), the resolved current state of every asset and relationship built from them, and the audit log described below. `GET /v1/observations`, `GET /v1/assets`, `GET /v1/relationships`, and `GET /v1/audit` all read from the same store, so a controller restart with `-db-driver sqlite` returns exactly what was there before.

**Still in-memory only, deliberately not addressed by this milestone:** enrollment tokens, collector heartbeats, and jobs. Each was already documented as in-memory-only when built (see `docs/enrollment.md`, `docs/heartbeats.md`, `docs/jobs.md`), and that has not changed here — persisting them is a question for a later slice once discovery-data and audit persistence are both proven in practice, not assumed now. A controller restart still requires re-enrollment tokens to be reissued, and any in-flight job or heartbeat history is lost, exactly as before. The audit log itself still records these actions (token issuance, enrollment, rotation, job creation) even though the underlying token/heartbeat/job state is not durable — the *record that the action happened* persists independently of whether the action's own transient state does.
...
Enrollment tokens, heartbeats, and job state remain in-memory only (see above). There is no backup/restore tooling beyond SQLite's own file-level tools, and no encryption at rest. The audit log is tamper-evident, not tamper-proof — see "Audit log" above for exactly what that guarantee does and does not cover, and note `GET /v1/audit` does not itself re-verify the chain. Server-side recurring discovery scheduling (today the controller only queues one-off jobs; recurring schedules are entirely client-side, via `topo agent run -interval`) is the last remaining slice of this milestone. See `docs/project-plan.md` for the full milestone spec.
