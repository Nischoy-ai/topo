...
Non-Lab targets must use HTTPS; Go's standard TLS hostname and certificate verification remains enabled. The CLI's Basic authentication mode is explicitly limited to loopback Topo Lab endpoints, and its password is read from an environment variable rather than a flag. Each CIM operation has a deadline, responses are bounded, enumeration pages and objects are capped, and target concurrency is controlled. Required identity/hardware failures reject that target; optional network, volume, service, or patch permission and parse failures retain a partial host and identify the affected operation. Built-in NTLM/Negotiate, registry software inventory, and real-system compatibility validation are not complete and must be added before an enterprise pilot.
