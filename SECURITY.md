...
Each CIM operation has a deadline, responses are bounded, enumeration pages and objects are capped, and target concurrency is controlled. Required identity/hardware failures reject that target; optional network, volume, service, or patch permission and parse failures retain a partial host and identify the affected operation. Registry software inventory and real-system compatibility validation are not complete and must be added before claiming the Windows milestone complete.
