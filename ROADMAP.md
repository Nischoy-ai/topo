...
- **Implemented and verified against a real instance:** the ServiceNow IRE payload builder deduplicates by `source_native_key` within a batch and emits stable identity across repeated scans. Real developer-instance runs now cover the complete supported boundary: `cmdb_ci_computer`, `cmdb_ci_network_adapter`, and `Owns::Owned by`, including a 22-item/21-relation laptop batch whose identical repeat returned only `NO_CHANGE`. Disk, software-package, and VM payloads produced real preflight dependency/key warnings and are deliberately rejected until focused mappings prove those contracts. Additional CI classes, relationship types, and the full IRE response schema remain unverified. See [docs/servicenow.md](docs/servicenow.md).
...
- **ServiceNow product direction — Topo discovery plus IRE:** Topo owns
  ...
  **Implemented in the current slice:** `topo publish servicenow` consumes
  bounded Topo observation JSON Lines, previews locally by default, requires an
  explicit `-apply` plus a credential reference to write, automatically runs
  ServiceNow's non-committing `queryEnhanced` preflight, retries only
  transport/429/5xx failures, and rejects unreviewed classes, relationships,
  warnings, and arbitrary CMDB fields. A real developer-instance run published
  one laptop plus 21 adapters and 21 relations; repeating it returned only
  `NO_CHANGE`, and the restricted OAuth token received 401 from an unrelated
  Table API. A real preflight also exposed required dependency/key contracts
  for disk, software, and VM classes, so the supported boundary is deliberately
...
- **In progress (first signed beta distribution):** the public
  ...
  signing and beta distribution environments are restricted to `main` with
  self-review prevention. A read-only, secret-name-only preflight is
  implemented and correctly blocks release while administrator bypass, a
  second reviewer, and the owner-provisioned RPM, Authenticode, Apple, notary,
  repository-signing, and least-privilege distribution credentials remain
  absent. No official release or channel is published yet.
