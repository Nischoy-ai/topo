...
- **In progress:** Persistent observation/audit storage and scheduling. Slice 1 (persistent storage) is implemented: `topo serve -db-driver sqlite -db-dsn <path>` opts into a SQLite-backed `store.Repository` (`internal/store/sqlite`, `modernc.org/sqlite`, pure-Go/no cgo) that survives a controller restart; `-db-driver memory` (default) keeps prior in-memory-only behavior unchanged. Relationships are now queryable through `store.Repository` for the first time (`ListRelationships`, `GET /v1/relationships`), and `SaveObservation` is idempotent by observation ID in both backends. A shared conformance test suite proves the two backends behave identically. Enrollment tokens, heartbeats, and job state remain in-memory only. PostgreSQL evaluated and deliberately deferred — no HA/clustered-controller story exists yet to justify a client-server database over a single embedded file. Slices 2 (immutable audit log) and 3 (server-side recurring scheduling) have not started. See [docs/storage.md](docs/storage.md).
