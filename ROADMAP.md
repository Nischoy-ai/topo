...
- **In progress:** Persistent observation/audit storage and scheduling. Slice 1 (persistent storage) is starting: a SQLite-backed `store.Repository` implementation (`internal/store/sqlite`, `modernc.org/sqlite`, pure-Go/no cgo) as an opt-in alternative to the in-memory-only default, plus relationships becoming queryable through `store.Repository` for the first time. PostgreSQL evaluated and deliberately deferred — no HA/clustered-controller story exists yet to justify a client-server database over a single embedded file. See [docs/project-plan.md](docs/project-plan.md).
